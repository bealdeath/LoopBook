...
export async function parseReceipt(req: Request, res: Response) {
  try {
    ...
    if (purchaseDate === "Unknown") {
      ...
      if (fallbackDate) purchaseDate = fallbackDate;
    }
    ...
    if (vendorName === "Unknown") {
      ...
      if (fallbackVendor) vendorName = fallbackVendor;
    }
 ...
 */
function detectCardLast4(text: string): string {
...
function detectDateWithDayMonthDisambiguation(text: string): string | null {
...
function monthNameToNumber(m: string): number {
...
function formatDateParts(yStr: string, mStr: string, dStr: string): string {
...
function detectFallbackVendor(text: string): string | null {
  const lower = text.toLowerCase();
  if (lower.includes("shell")) return "Shell";
  if (lower.includes("costco")) return "Costco";
  if (lower.includes("ikea")) return "IKEA";
  if (lower.includes("l'avenue")) return "L'Avenue Toronto";
  if (lower.includes("the morning after")) return "The Morning After";
  if (lower.includes("john andersons")) return "John Anderson's Hamburgers";
  return null;
}
 ...
 */
function tryParseLineItems(rawText: string): {
...
function findLargestDollarAmount(text: string): number {
  const matches = text.match(/\$?\d+\.\d{1,2}/g) || [];
  let max = 0;
  for (const raw of matches) {
    if (raw.includes("-")) continue; // skip phone-like
